...
def draw_bounding_box_on_image(current_frame_number,image,
  ...
  # Draw centroid point
  draw.ellipse((abs(((right+left)/2)), abs(((top+bottom)/2)), abs(((right+left)/2))+10, abs(((top+bottom)/2))+10), fill = 'blue', outline ='blue')
  ...
  # print(top, " ==== ", ROI_POSITION)
  
  # if(top > ROI_POSITION): # if the vehicle get in ROI area, vehicle predicted_speed predicted_color algorithms are called - 200 is an arbitrary value, for my case it looks very well to set position of ROI line at y pixel 200
  #       print("")
  ...
  if(x_axis[0] == 1):
    ...
    predicted_direction, Lane, is_vehicle_detected, update_csv = object_counter_x_axis.count_objects_x_axis(top, bottom, right, left, temp, start_time, current_frame_number, detected_vehicle_image, ROI_POSITION[0], ROI_POSITION1[0], ROI_POSITION[0]+DEVIATION[0], ROI_POSITION[0]+(DEVIATION[0]*2), DEVIATION[0])
  elif(mode_number[0] == 2):
    start_time = int(time.time()*1000.0)
    predicted_direction, Lane, is_vehicle_detected, update_csv, VehicleType = object_counter.count_objects(top, bottom, right, left, start_time, current_frame_number, detected_vehicle_image, ROI_POSITION[0], ROI_POSITION1[0], ROI_POSITION[0]+DEVIATION[0], ROI_POSITION[0]+(DEVIATION[0]*2), DEVIATION[0])
...
def visualize_boxes_and_labels_on_image_array(current_frame_number,image,
  ...
  counter = 0
  is_vehicle_detected = []
  mode_number.insert(0,2)
  ROI_POSITION.insert(0,y_reference)
  ROI_POSITION1.insert(0,roi_Lane2)
  ...
  for box, color in box_to_color_map.items():
    ...
    # if (("Number Plate (Car)" in display_str_list[0])):
    if (("Number_Plate" in display_str_list[0])):
