...
from utils.speed_and_direction_prediction_module import speed_prediction_v
...
def draw_bounding_box_on_image(current_frame_number,
  ...
  if(left > ROI_POSITION): # if the vehicle get in ROI area, vehicle predicted_speed predicted_color algorithms are called - 200 is an arbitrary value, for my case it looks very well to set position of ROI line at y pixel 200
    print("")
    predicted_direction, predicted_speed,  is_vehicle_detected, update_csv = speed_prediction_v.predict_speed(top, bottom, right, left, current_frame_number, detected_vehicle_image, ROI_POSITION)
...
def visualize_boxes_and_labels_on_image_array(current_frame_number,image,
  ...
  return counter, csv_line_util, temp
